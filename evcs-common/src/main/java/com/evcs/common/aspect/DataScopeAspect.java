...
public class DataScopeAspect {
    ...
    private void checkTenantHierarchyPermission(JoinPoint joinPoint, DataScope dataScope, Long currentTenantId) {
        ...
        if (hierarchyValidator != null) {
            if (hierarchyValidator.validateAccess(currentTenantId, targetTenantId)) {
                return;
            }
        } else {
            // Fallback to TenantContext (Note: verify strictly if possible)
            if (TenantContext.hasAccessToTenant(targetTenantId)) {
                return;
            }
        }
    ...
    private void checkRolePermission(JoinPoint joinPoint, DataScope dataScope) {
        ...
        //
        // // 根据角色的数据权限范围过滤数据
        // String dataScope = roleInfo.getDataScope(); // 例如：ALL, DEPT, DEPT_AND_CHILD,
        // SELF
    ...
    private Long extractTenantIdFromArgs(JoinPoint joinPoint) {
        ...
        for (int i = 0; i < parameters.length; i++) {
            ...
            // 检查参数名是否包含tenantId或parentId
            String paramName = parameter.getName().toLowerCase();
            if ((paramName.contains("tenantid") || paramName.contains("parentid")) && arg instanceof Long) {
                return (Long) arg;
            }
