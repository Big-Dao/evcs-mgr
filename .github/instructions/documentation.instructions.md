...
本规范是 [AGENTS.md](../../AGENTS.md) 中定义的 SSOT 体系的一部分。
...
当 AI 助手需要创建或修改任意 Markdown 文档时，必须严格遵守以下流程：

1. **先读规范**：阅读并遵循本规范；涉及架构/编码/多租户/异步等内容时，必须以 SSOT 为准（见 `AGENTS.md` 与 `docs/overview/PROJECT-CODING-STANDARDS.md`）。
2. **先定位权威来源**：优先修改“唯一权威文档”，其它文档只做引用链接，避免事实在多处重复维护（DRY）。
3. **改动后自检**：变更完成后必须运行文档风格检查（CI 同步执行），确保符合：
   - 章节标题使用数字编号（H2+）
   - 禁止装饰性 emoji（仅允许状态表达/强调/风险提示）
...
**目录原则**：
- 不在子目录维护 `README.md`（避免重复入口与长期漂移；**例外**：允许 `docs/archive/README.md` 作为“归档区说明/免责声明”）
...
4. **添加视觉辅助**
   - **禁止使用装饰性 emoji**；仅允许用于“状态表达/强调/风险提示”（例如 ✅/❌/⚠️）
   - 使用引用块突出重要信息
   - 使用代码块标注语言：\`\`\`java, \`\`\`bash
...
`docs/archive/**` 视为“历史快照区”，默认不做持续维护。

**基本原则**：
- **历史文档不修链**：`docs/archive/**` 中早于“本次归档操作”产生的历史快照，允许存在断链与过时引用，不进行批量修复。
- **本次归档必须修链**：在“当次归档操作”中被移动/新增到 `docs/archive/**` 的文档，**必须**在同一次变更里完成断链修复，使其在新位置可自洽阅读。
- **不把 archive 当入口**：权威入口仍然是 `docs/DOCUMENTATION-INDEX.md`；归档区仅用于追溯与审计。

**归档时必须做的最小动作**：
1. 在归档文档开头添加“已归档/历史快照”声明，并注明归档日期与原因（可链接到当次的 `CLEANUP-PLAN-YYYY-MM-DD.md` / `CLEANUP-SUMMARY-YYYY-MM-DD.md`）。
2. 修正该文档内部的相对链接（仅限该文档自身引用的文件/图片/同批归档文档）。
3. 不修改其它历史归档文档以“顺带修链”（避免把 archive 变成长期维护负担）。
...
1. **仅对本次归档修链**
   - 必须在同一批变更中修复“本次新归档文档”的断链。
   - 不要为了让 `docs/archive/**` 全量通过死链检查而去修改历史快照。

2. **归档文档必须声明“历史快照”**
   - 在文档顶部添加归档声明（归档日期、原因、可选：指向当次归档计划/总结）。

3. **死链检查范围默认排除 archive**
   - 日常死链检查以非归档区为准（例如排除 `docs/archive/**`）。
   - 当次归档涉及的文件例外：必须在提交前确保本次归档文件不新增断链。
