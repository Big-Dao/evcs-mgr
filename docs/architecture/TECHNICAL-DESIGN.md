...
### 3.5 C 端用户模块设计约束（evcs-user）

用户模块设计详见 [evcs-user 模块 RFC](EVCS-USER-MODULE-RFC.md)，此处仅列出关键设计约束：

#### 3.5.1 用户与 B 端用户解耦

- C 端用户（`charging_user`）与 B 端用户（`sys_user`）完全独立，不共用表与账户体系
- C 端用户 token 与 B 端用户 token 采用不同签发策略，网关层区分路由

#### 3.5.2 多租户与用户归属

- 每个 C 端用户必须归属于某个租户（`tenant_id`）
- 用户数据遵循租户隔离，运营商只能管理本租户用户
- 若存在"用户可在多租户间共享"的场景，需显式设计"用户-租户关联表"

#### 3.5.3 积分/余额/优惠券一致性

- 积分/余额变更必须记录流水，支持审计与对账
- 优惠券核销必须幂等，不可重复使用
- 订单取消时优惠券需支持释放或标记失效

#### 3.5.4 营销活动与权益

- 活动预算与名额必须原子扣减（Redis + DB 双写或分布式锁）
- 权益包订阅状态需定时检查过期
...
- 画像数据按日/周聚合，不实时计算
- 行为事件表建议按时间分区
- 敏感信息（手机号、身份证）必须脱敏存储与展示

#### 3.5.6 账户安全与风险控制

- 用户识别采用 **OAuth 优先 + 手机号备用** 的多因子策略
- 手机号销号风险防范：OAuth 绑定校验、活跃度检测、敏感操作二次验证
- 换绑手机号需通过原手机号/OAuth/人脸验证
- 支持多设备登录（默认最多 5 台），设备 90 天未活跃自动失效
- 风险登录检测与标记

#### 3.5.7 用户信息完整性

- 支持多种证件类型（身份证/护照/港澳通行证/台胞证/军官证/驾驶证/外国人永居证）
- 多渠道联系方式（手机/邮箱/微信/QQ/企业微信/钉钉/Telegram/WhatsApp）
- 完整地址信息（国家/省/市/区/详细地址/邮编）
- 职业信息、紧急联系人、偏好设置（语言/时区/货币/通知开关）
- 快捷引用（默认车辆/默认支付方式/默认发票抬头）
- 推荐关系、VIP/黑名单标记、生命周期追踪

## 4. 相关入口

- 文档统一入口：[docs/DOCUMENTATION-INDEX.md](../DOCUMENTATION-INDEX.md)
- evcs-user 模块 RFC：[docs/architecture/EVCS-USER-MODULE-RFC.md](EVCS-USER-MODULE-RFC.md)
