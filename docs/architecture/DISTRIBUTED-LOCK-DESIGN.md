...
| 场景 | 锁 Key | 说明 |
|------|--------|------|
| 充电桩启动 | `lock:charger:start:{chargerId}` | 充电桩启动锁 |
| 充电桩停止 | `lock:charger:stop:{chargerId}` | 充电桩停止锁 |
| 充电桩操作 | `lock:charger:{chargerId}` | 充电桩通用锁 |
| 订单状态变更 | `lock:order:status:{orderId}` | 订单状态锁 |
| 支付操作 | `lock:payment:{orderNo}` | 支付锁 |
| 用户余额 | `lock:user:balance:{userId}` | 用户余额锁 |
| 定时任务 | `lock:scheduler:{taskName}` | 调度任务锁 |
...
public class ChargerService {
    ...
    public void startCharging(Long chargerId, Long userId) {
        String lockKey = "lock:charger:" + chargerId;
        ...
        try {
            ...
            if (lock.tryLock(5, 30, TimeUnit.SECONDS)) {
                try {
                    // 执行业务逻辑
                    doStartCharging(chargerId, userId);
                } finally {
                    // 确保释放锁
    ...
    public Object around(ProceedingJoinPoint joinPoint, DistributedLock distributedLock) throws Throwable {
        ...
        try {
            if (lock.tryLock(distributedLock.waitTime(), distributedLock.leaseTime(), TimeUnit.SECONDS)) {
                ...
                throw new BusinessException(distributedLock.errorMessage());
            }
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new SystemException("获取锁被中断", e);
...
public void safeOperation(Long resourceId) {
    ...
    try {
        ...
        if (!acquired) {
            throw new BusinessException("资源正在被其他操作占用");
        }
        ...
        Thread.currentThread().interrupt();
        throw new SystemException("操作被中断", e);
    } catch (Exception e) {
        log.error("操作失败", e);
        throw e;
    } finally {
