...
冻结说明：上述 `iss/aud` 作为默认冻结值；不同环境可通过配置覆盖，但必须保持 C/B 两套 token 的硬隔离（不同 `iss`、不同 `aud`、不同签名 key）。
...
- 在迁移阶段 1 内，网关与业务服务 SHOULD 兼容解析 legacy claims（例如 `userId` 作为 `sub` 的别名）。
- 新签发的 token MUST 使用本 RFC 约定的标准 claims（`sub/tenantId/userType/jti/iat/exp`），以便后续引入多 issuer/audience 与 key 轮换。
...
| `userId` | Long | legacy 用户 ID（迁移期兼容：等价于 `sub`） | 签发器写入 | 迁移期 |
