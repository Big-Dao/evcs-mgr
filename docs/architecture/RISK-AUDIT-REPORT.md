...
| 问题场景 | 影响 | 当前状态 |
|----------|------|----------|
| 并发启动/停止同一充电桩 | 状态不一致，可能导致重复计费 | ❌ 未实现 |
| 并发支付同一订单 | 可能导致重复扣款 | ⚠️ 仅幂等检查，无锁保护 |
| 并发更新同一充电枪状态 | 状态覆盖，数据不一致 | ❌ 未实现 |
...
| 机制 | 用途 | 当前状态 | 影响 |
|------|------|----------|------|
| 熔断器 (Circuit Breaker) | 防止级联故障 | ❌ 0 处使用 | 支付网关故障会拖垮订单服务 |
| 限流器 (Rate Limiter) | 防止流量突增 | ❌ 0 处使用 | 无法防止 DDoS 或恶意请求 |
| 重试策略 (Retry) | 处理瞬态故障 | ❌ 0 处使用 | 网络抖动直接失败 |
...
### 2.3 部分 Controller 缺少权限验证

**风险描述**: 多个 Controller 未配置 `@PreAuthorize` 注解，存在越权访问风险。

| Controller | 模块 | 风险级别 | 影响 |
|------------|------|----------|------|
| `TenantController` | evcs-tenant | 🔴 高 | 租户管理可被未授权访问 |
| `ProtocolCommandController` | evcs-protocol | 🔴 高 | 设备控制指令可被未授权发送 |
| `ReconciliationController` | evcs-payment | 🔴 高 | 资金对账数据可被未授权访问 |
| `ProtocolDebugController` | evcs-protocol | 🔴 高 | 调试接口暴露敏感信息 |
| `PaymentCallbackController` | evcs-payment | 🟠 中 | 需签名验证替代权限验证 |
| `DashboardController` | evcs-tenant | 🟠 中 | 统计数据可被未授权访问 |
