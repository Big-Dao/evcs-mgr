# 文档快速导航（指针）

> 一句话说明：当你不确定该看哪份文档时，从这里快速跳转到权威入口；完整导航以 docs/DOCUMENTATION-INDEX.md 为准。

**最后更新**: 2025-12-17  \
**维护者**: PMO / 技术负责人  \
**状态**: 已发布

---

## 30 秒定位入口

- 文档总索引（推荐）：[docs/DOCUMENTATION-INDEX.md](../DOCUMENTATION-INDEX.md)
- 部署（按场景选入口）：[docs/deployment/README.md](../deployment/README.md)
- 开发： [docs/development/DEVELOPER-GUIDE.md](../development/DEVELOPER-GUIDE.md)
- 测试： [docs/testing/README.md](../testing/README.md)
- 故障排查： [docs/troubleshooting/README.md](../troubleshooting/README.md)

## 常见问题（最短答案）

- 如何快速启动？

- 本地开发/演示：见 [docs/deployment/QUICK-START.md](../deployment/QUICK-START.md)
- CI/测试环境：见 [docs/deployment/TEST-ENVIRONMENT-GUIDE.md](../deployment/TEST-ENVIRONMENT-GUIDE.md)

- 默认账号密码在哪里？

- 见 [docs/operations/DEFAULT-CREDENTIALS.md](../operations/DEFAULT-CREDENTIALS.md)

- 遇到配置/启动问题怎么办？

- 先看 [docs/troubleshooting/ERROR_PREVENTION_CHECKLIST.md](../troubleshooting/ERROR_PREVENTION_CHECKLIST.md)
