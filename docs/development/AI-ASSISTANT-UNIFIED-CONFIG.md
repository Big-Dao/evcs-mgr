...
- 修改 `**/*.md` 前必须阅读并遵循 `.github/instructions/documentation.instructions.md`。
- 提交前建议运行：`python3 scripts/check_markdown_style.py`（章节数字编号、禁止装饰性 emoji；CI 会对改动的 Markdown 执行同等检查）。
