# EVCS数据库设计规�?
> **版本**: v1.1 | **最后更�?*: 2025-11-10 | **维护�?*: 数据架构�?| **状�?*: 活跃
>
...
| 服务 | 数据�?| 用�?|
|------|--------|------|
| evcs-auth | evcs_auth | 用户认证、权限管�?|
| evcs-station | evcs_station | 充电站、充电桩管理 |
| evcs-order | evcs_order | 订单、充电记�?|
| evcs-payment | evcs_payment | 支付记录、财�?|
...
| VARCHAR(n) | 短文�?| `name VARCHAR(100)` |
| TEXT | 长文�?| `description TEXT` |
| DECIMAL(p,s) | 金额 | `amount DECIMAL(12,2)` |
| TIMESTAMP | 时间�?| `created_at TIMESTAMP` |
...
-- 百分比字�?rate DECIMAL(5,2) NOT NULL CHECK (rate >= 0 AND rate <= 100)
...
-- 状态字�?status VARCHAR(20) NOT NULL CHECK (status IN ('ACTIVE', 'INACTIVE', 'PENDING'))
```
...
-- 在应用层设置租户上下�?SET app.current_tenant_id = '123';
```

### 租户相关索引
```sql
-- 租户相关查询的复合索�?CREATE INDEX idx_orders_tenant_status ON orders(tenant_id, status);
...
-- 充电站可用性查�?CREATE INDEX idx_stations_status_capacity
...
-- 创建审计触发�?CREATE OR REPLACE FUNCTION audit_trigger_function()
RETURNS TRIGGER AS $$
BEGIN
...
### 版本控制
```sql
-- 创建迁移版本�?CREATE TABLE schema_migrations (
...
BEGIN
    ...
    -- 创建测试充电�?    INSERT INTO stations (tenant_id, station_no, name, address, capacity)
    VALUES (tenant_id_param, 'TEST001', '测试充电�?, '测试地址', 10)
...
### 性能监控查询
```sql
-- 查看慢查�?SELECT query, calls, total_time, mean_time, rows
...
-- 查看表大�?SELECT schemaname, tablename,
